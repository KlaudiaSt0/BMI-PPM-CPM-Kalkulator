...
const GetInspired = ({insp}) => {
    ...
    if(insp === true) {
        return <div><h3 className="header_3">Urozmaicona dieta to podstawa zdrowia! Kiedy ostatnio w Twojej diecie znalazły się:</h3>
            <ul className="list">{recipesArray.map((el, index) => <li key={index}> <b>{el.recipe.label}</b> - meal type: {el.recipe.mealType}, kcal: {(el.recipe.calories).toFixed(2)} kcal</li>)}</ul></div>
    } else {
        return null;
    }
}
